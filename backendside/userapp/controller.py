...
def getRegistrationByUserCode(ruser, code):
    ...
    try:
        regis = Registration.objects.get(registration_user=code,registration_confirm_code=code)
        result[RESULT]= regis
        return result
    except:
        print("Register-controller-getRegistrationByIDCode: ", sys.exc_info()[0])
        result[STATUS]=ERROR
...
def getRegistrationByStatusAndUser(ruser, status):
    ...
    try:
        regis = Registration.objects.get(registration_user=code,registration_status=WAITING).order_by('registration_date')
        result[RESULT]= regis
        return result
    except:
        print("Register-controller-getRegistrationByIDCode: ", sys.exc_info()[0])
...
def confirmRegistrationByUserCodeMass(ruser, code, mass):
    ...
    try:
        registrations = Registration.objects.filter(registration_user=ruser
                                                  ,registration_confirm_code=code
                                                  ,registration_mass=mass)
        ...
        print("Register-controller-confirmRegistrationByUserCodeMass: ", sys.exc_info()[0])
        result[STATUS]=ERROR
        result[CONTENT]= SYSTEM_ERROR_0001
        return result
...
def checkIn(regis_id, regis_code):
    ...
    try:
        registration = Registration.objects.get(pk=regis_id,registration_confirm_code=regis_code)
        if registration:
            ...
            if registration.registration_status == APPROVED:
                ...
                result[CONTENT]['user']= {"username":user_name,"seat":seat_num}
...
def cancelRegistration(r_id,code):
    ...
    try:
        registration = Registration.objects.get(pk=r_id,registration_confirm_code=code)
        if registration:
            if registration.registration_status == APPROVED:
                ...
                if today.weekday() >= THURDAY and today.weekday()< SATURDAY:      # tranfer it to waiting people
                    mass = registration.registration_mass
                    w_registration = Registration.objects.filter(registration_mass= mass
                                                                ,registration_status=WAITING).order_by('registration_date').first()
                    if w_registration:
...
def checkUserRegistered(p_user,p_relation,p_mass):
    ...
    try:
        registrations = Registration.objects.filter(registration_user=p_user,registration_mass=p_mass)
        today = datetime.today()
        for registration in registrations:
            if registration:
        ...
        if p_mass.mass_language == VI:
            ...
            last_mass = Mass.objects.filter(mass_date_ordinary=previous_week,mass_language = VI).order_by('-mass_date').first()
            lastregis = Registration.objects.filter(registration_user=p_user,registration_mass=last_mass).order_by('-registration_date').first()
            if lastregis:
            ...
            next_mass = Mass.objects.filter(mass_date_ordinary=next_week,mass_language = VI).order_by('-mass_date').first()
            next_regis = Registration.objects.filter(registration_user=p_user,registration_mass=next_mass).order_by('-registration_date').first()
            if next_regis:
...
def getNextMassesByChurch(p_church,p_status):
    ...
    try:
        ...
        masses = Mass.objects.filter(mass_church=p_church,mass_active=p_status).order_by("mass_time")    #mass_date__gte = timezone.now()
...
def getActiveMasses(p_status):
    ...
    try:
        masses = Mass.objects.filter(mass_active=p_status)    #mass_date__gte = timezone.now()
        result[RESULT] = masses
        return result
    except:
        print("Register-controller-: ", sys.exc_info()[0])
...
def get_seat_for_50(mass_schedule):
    seat = Seat.objects.filter(seat_type=SEAT_50,seat_mass_schedule=mass_schedule,seat_status=AVAILABEL).order_by('seat_no').first()
    return seat
def get_seat_for_32(mass_schedule):
    seat = Seat.objects.filter(seat_type=SEAT_32,seat_mass_schedule=mass_schedule,seat_status=AVAILABEL).order_by('seat_no').first()
    return seat
def get_seat_for_10(mass_schedule):
    seat = Seat.objects.filter(seat_type=SEAT_20,seat_mass_schedule=mass_schedule,seat_status=AVAILABEL).order_by('seat_no').first()
    return seat
def get_seat_for_priority(mass_schedule):
    seat = Seat.objects.filter(seat_type=SEAT_PRI,seat_mass_schedule=mass_schedule,seat_status=AVAILABEL).order_by('seat_no').first()
    return seat
...
def getSeatNoForGroup(mass_schedule,num_seat):
    seats = Seat.objects.filter(seat_mass_schedule=mass_schedule,seat_status=AVAILABEL).order_by('seat_no')[:num_seat]
    return seats
...
def sigleRegister(mass_id,user_condi,user):
    ...
    registration = Registration(registration_user=user,registration_mass=mass,registration_user_name=users_profile.profile_full_name)
    ...
    #print(ALLOWED_HOSTS)
    registration.registration_code = HOST_NAME+"register/cfmatt/?"+USER_ID+"="+str(user.pk)+"&"+BOOKING_ID+"="+str(registration.pk)+"&"+MASS_ID+"="+str(mass_id)+"&"+CODE+"="+random_code
    registration.save()
    ...
    seatContent = getSeatNo(users_profile.profile_age,mass_schedule,NOTPRIORITY)
    #minus 1 slot
    if seatContent[STATUS]  == OK:
...
def multipleRegister(mass_id,user_condi,user,list_u):
    ...
    # Save Registration
    from .models import Registration
    registration = Registration(registration_user=user,registration_mass=mass,registration_user_name=users_profile.profile_full_name,registration_user_age=users_profile.profile_age)
    ...
    for user_re in list_u:
        registration = Registration(registration_user=user,registration_mass=mass,registration_user_name=user_re["uname"],registration_user_age=user_re["uage"])
        random_code = get_random_string(length=18, allowed_chars=CODERANGE)
        registration.registration_confirm_code=random_code
        registration.save()
        #print(booking.booking_confirm_code)
