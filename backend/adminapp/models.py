...
class UserProfile(models.Model):
    ...
    profile_image = models.ImageField(_('Hình đại diện'),default='',blank=True,null=True, upload_to='media/profile_pics',help_text=_('Không bắt buộc'))
    ...
    profile_health_status = models.CharField(_('Tình trạng'),max_length=15,choices=health_choice,default="GOOD",help_text=_('Tình trạng sức khoẻ của bạn hiện tại'))
    profile_last_check_tempture = models.SmallIntegerField(_('Nhiệt độ cơ thể'),default=36,help_text=_('Nhiệt độ cơ thể lần kiểm tra gần nhất'))
...
class Church(models.Model):
    ...
    church_language_main = models.CharField(_('Ngôn ngữ'),help_text=_('Ngôn ngữ'),max_length=15,choices=language_choice)
...
class ChurchPost(models.Model):
    ...
    church_post_language = models.CharField(_('Ngôn ngữ'),max_length=15,choices=language_choice,default="vi",help_text=_('Ngôn ngữ sử dụng'))
...
class DailyGospel(models.Model):
    daily_gospel_title = models.CharField(_('Chủ đề'),help_text=_('Chủ đề của ngày '),max_length=50)
    daily_gospel_date = models.DateField(_('Ngày tháng'),help_text=_('Ngày tháng dương lịch'))
    daily_gospel_date_ordinary = models.CharField(_('Tuần phụng vụ'),help_text=_('Tuần phụng vụ'),max_length=3)
    daily_gosspel_content = HTMLField(_('Nội dung'),help_text=_('Lời Chúa Nội dung'),default='',null=True,blank=True)
    daily_gospel_laguage = models.CharField(_('Ngôn ngữ'),max_length=15,choices=language_choice,default="vi",help_text=_('Ngôn ngữ sử dụng'))
    daily_gospel_readed = models.IntegerField(_('Số lượt đọc'),help_text=_('Số lượt đọc'),default=0)
    daily_gospel_created_date = models.DateTimeField(_('Ngày viết'),help_text=_('Ngày tạo'),default=timezone.now)
    daily_gospel_last_updated_date = models.DateTimeField(_('Ngày cập nhật'),help_text=_('Lần cuối cập nhật'),default=timezone.now)
    daily_gospel_created_user = models.ForeignKey(User, on_delete=models.CASCADE,null=True,blank=True)
    daily_gospel_like = models.IntegerField(_('Post like'),help_text=_('Number of post liked'),default=0)
    daily_gospel_share = models.IntegerField(_('Posted Shared'),help_text=_('Number of post shared'),default=0)
    daily_gospel_clicked = models.IntegerField(_('View numbers'),help_text=_('Viewed numbers'),default=0)
    daily_gospel_status = models.BooleanField(_('Status'),help_text=_('Approve status'),default=True,blank=True,null=True)
...
class Mass(models.Model):
    ...
    mass_reading = models.ForeignKey(DailyGospel, on_delete=models.CASCADE,blank=True,null=True)
    ...
    mass_father_celebrant = models.ForeignKey(User,verbose_name=_('Cha chủ tế'),help_text=_('Cha chủ tế'),on_delete=models.CASCADE,blank=True,null=True,related_name="mass_father")
    ...
    mass_total_registered = models.SmallIntegerField(_('Tổng số đăng ký'),help_text=_('Tổng số người đăng ký bao gồm cả người huỷ'),default=0,blank=True,null=True)
    ...
    mass_created_user = models.ForeignKey(User, on_delete=models.CASCADE,help_text=_('Người cuối cập nhật'),blank=True,null=True,related_name="created_user")
    mass_require_flag = models.BooleanField(_('Yêu cầu đăng ký '),help_text=_('Yêu cầu đăng'),default=True,blank=True,null=True)
    mass_active = models.BooleanField(_('Tình trạng'),help_text=_('Cho phép đăng ký'),default=True,blank=True,null=True)
...
class MassTime(models.Model):
    ...
    mass_date = models.DateTimeField(_('Thời gian'),help_text=_('Thời điểm bắt đầu'),null=True,blank=True)
    mass_time = models.DateTimeField(_('Thời gian'),help_text=_('Thời điểm kết thúc'),null=True,blank=True)
    mass_time_language = models.CharField(_('Ngôn ngữ'),max_length=15,choices=language_choice,default="vi",help_text=_('Ngôn ngữ sử dụng'))
    ...
    mass_time_status = models.BooleanField(_('Status'),help_text=_('Approve status'),default=True,blank=True,null=True)
...
class Event(models.Model):
    ...
    event_place = models.ForeignKey(Church,on_delete=models.CASCADE, help_text=_('Nơi tổ chức'),blank=True,null=True)
